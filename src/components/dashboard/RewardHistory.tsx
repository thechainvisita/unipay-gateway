...
export default function RewardHistory({ data = [] }: RewardHistoryProps) {
  ...
  const tableData = data.map((his) => ({
    ...
    reward_source: (
      <span className="text-white font-aeonik whitespace-nowrap">{his.source}</span>
    ),
    tokens: (
      <span className="text-emerald-400 font-aeonik whitespace-nowrap">{his.tokens}</span>
    ),
    note: <span className="text-white/70 text-sm whitespace-nowrap">{his.note}</span>,
  }));
    ...
    <section className="w-full max-w-8xl mx-auto mb-10 text-white">
      <div className="flex justify-between items-center mb-6">
        ...
        <p className="text-sm font-aeonik text-cyan-400">
          PAY earned so far <span className="text-emerald-400">{totalTokens}</span> Tokens
        </p>
      </div>
