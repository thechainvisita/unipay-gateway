...
export default function PurchaseHistory({ data = [] }: PurchaseHistoryProps) {
  ...
  const filteredData = data
    .filter((item) => (methodFilter === 'All' ? true : item.payment_method === methodFilter))
    ...
    .map((his) => ({
      ...
      merchant: (
        <span className="text-white font-aeonik whitespace-nowrap">{his.merchant_name}</span>
      ),
      item: <span className="text-white/80 whitespace-nowrap">{his.item}</span>,
      amount_paid: (
        <span className="text-white whitespace-nowrap">${his.amount_paid.toFixed(2)}</span>
      ),
      payment_method: (
        <span className="text-white whitespace-nowrap">{his.payment_method}</span>
      ),
      status: (
        <span
      ...
      <div className="w-full overflow-x-auto">
        <div className="border border-white/10 rounded-2xl overflow-hidden shadow-inner">
          ...
          <div className="bg-gradient-to-r from-cyan-400 to-blue-500 text-white text-sm font-aeonik px-6 py-3 rounded-t-2xl grid grid-cols-7 gap-4">
            {columns.map((col, idx) => (
              <div key={idx} className="whitespace-nowrap text-center overflow-hidden">
                {col}
              </div>
            ))}
          </div>
