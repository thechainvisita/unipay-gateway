...
export default function AccountInfo({ user }: AccountInfoProps) {
  const info = [
    ...
    { label: 'Name', value: user.user_metadata?.name || user.email, status: 'Set' },
  ];
    ...
    <section className="w-full max-w-2xl mx-auto text-white">
      ...
      <div className="space-y-3">
        {info.map((item, index) => (
          ...
          >
            <span className="text-sm font-aeonik text-white/90">{item.label}</span>
            <div className="text-right">
              <div className="text-sm text-emerald-400 font-aeonik">{item.value}</div>
