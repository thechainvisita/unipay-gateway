...
export default function PaymentMethodSelector() {
  ...
  useEffect(() => {
    // User data is available via localStorage when needed
  }, [status]);

  const handleSelectMethod_1 = (method: string) => {
    dispatch(setPaymentMethod_1(method));
    if (method === 'crypto') navigate('/payment-crypto');
    else navigate('/payment-fiat');
  };
        ...
        {[...Array(4)].map((_, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="font-bold text-cyan-300 text-2xl">{`0${i + 1}`}</span>
            </div>
          );
        })}
      </div>
